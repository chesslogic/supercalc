...
function buildAttackRecommendation({
  ...
  const result = filteredCandidates.length === 0
    ...
    : {
      ...
      // Margin qualification must describe the displayed candidate so sorting and the Margin cell agree.
      qualifiesForMargin: Boolean(bestCandidate?.qualifiesForMargin),
      hasAnyQualifyingMarginPath: filteredCandidates.some((candidate) => candidate.qualifiesForMargin),
      ...
      qualifiesForNearMiss: Boolean(bestCandidate?.qualifiesForNearMiss),
      hasAnyQualifyingNearMissPath: filteredCandidates.some((candidate) => candidate.qualifiesForNearMiss),
...
function buildWeaponRecommendationDisplayRow({
  ...
  return {
    ...
    hasAnyQualifyingMarginPath: Boolean(bestAttackRecommendation.hasAnyQualifyingMarginPath),
    ...
    hasAnyQualifyingNearMissPath: Boolean(bestAttackRecommendation.hasAnyQualifyingNearMissPath),
        ...
        buildRecommendationAttackPackages(weapon, {
          ...
          .map((recommendation) => {
            ...
            return {
              ...
              marginPercent: bestCandidate?.marginPercent ?? null,
              qualifiesForMargin: Boolean(bestCandidate?.qualifiesForMargin),
              hasAnyQualifyingMarginPath: recommendation.candidates.some((candidate) => candidate.qualifiesForMargin),
              displayMarginRatio: bestCandidate?.displayMarginRatio ?? null,
              displayMarginPercent: bestCandidate?.displayMarginPercent ?? null,
              nearMissRatio: bestCandidate?.nearMissRatio ?? null,
              nearMissPercent: bestCandidate?.nearMissPercent ?? null,
              qualifiesForNearMiss: Boolean(bestCandidate?.qualifiesForNearMiss),
              hasAnyQualifyingNearMissPath: recommendation.candidates.some((candidate) => candidate.qualifiesForNearMiss),

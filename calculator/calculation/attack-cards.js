...
import { isBeamAttack } from '../attack-types.js';
...
function appendAttackApplication(leftContent, application, {
  ...
  damageValue.textContent = `${application.zoneName}: ${formatDamageValue(application.zoneDamage)} zone${usesBeamCadence ? ' DPS' : ''}`;
  ...
  if (application.totalMainDamage > 0) {
    ...
    mainDamageValue.textContent = `Main: ${formatDamageValue(application.totalMainDamage)}${usesBeamCadence ? ' DPS' : ''} (${formatDamageValue(application.directMainDamage)} direct + ${formatDamageValue(application.passthroughMainDamage)} passthrough)`;
...
export function appendAttackCard(container, slot, attack, attackKey, index, {
  ...
  const usesBeamCadence = isBeamAttack(attack?.attack);
  ...
  targetSummary.textContent = attack.mode === 'explosion'
    ...
    : `${usesBeamCadence ? 'Beam' : 'Projectile'} target: ${attack.projectileTargetZoneName || 'none'}`;
  ...
  attackTotals.textContent = `${usesBeamCadence ? 'DPS' : 'Cycle'} total: ${formatDamageValue(attack.totalZoneDamagePerCycle)} zone • ${formatDamageValue(attack.totalDamageToMainPerCycle)} main`;
